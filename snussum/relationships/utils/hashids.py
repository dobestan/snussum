...
from snussum.settings import HASHIDS_DATING_SALT, HASHIDS_COMMENT_SALT, \
    HASHIDS_SELF_DATING_SALT, HASHIDS_SELF_DATING_APPLY_SALT
...
def get_self_dating_apply_hashid_object():
    return Hashids(salt=HASHIDS_SELF_DATING_APPLY_SALT, min_length=4)


def get_encoded_self_dating_apply_hashid(id):
    hashid = get_self_dating_apply_hashid_object()
    return hashid.encode(id)


def get_decoded_self_dating_apply_hashid(hash_id):
    hashid = get_self_dating_apply_hashid_object()
    return hashid.decode(hash_id)[0]
